...
**Corollaire** : si tu as besoin d'exporter une fonction utilitaire sync (ex: `hasTPAccess`), la mettre dans `lib/` — jamais dans un fichier `"use server"`.
**Corollaire 2** : les fichiers `app/api/**/route.ts` ne doivent JAMAIS avoir `"use server"` — ce sont des routes API, pas des server actions.
