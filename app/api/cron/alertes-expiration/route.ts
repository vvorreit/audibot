  ...
  for (const joursAvant of ALERTES_JOURS) {
    ...
    for (const dossier of dossiers) {
      ...
      await createNotification(
        dossier.userId,
        "alerte_expiration",
        `Ordonnance expirant dans ${joursAvant} jours`,
        `Dossier ${dossier.reference} (${dossier.mutuelle}) — échéance le ${dateExpiration.toLocaleDateString("fr-FR")}`,
        "/tiers-payant/alertes"
      ).catch((err) => console.error("[cron/alertes-expiration] Erreur notif:", err));
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;padding:32px 16px;color:#1e293b;">
  <h1 style="font-size:20px;font-weight:800;margin-bottom:8px;">Alerte expiration ordonnance</h1>
  <p style="font-size:14px;line-height:1.6;color:#475569;">
    L'ordonnance du dossier <strong>${dossier.reference}</strong>
  ...
  <p style="font-size:14px;line-height:1.6;color:#475569;">
    Pensez a contacter le client pour un renouvellement.
  </p>
