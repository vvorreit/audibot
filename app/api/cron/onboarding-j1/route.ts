...
export async function GET(req: Request) {
  ...
  for (const user of users) {
    ...
    try {
      ...
      await sendMail({
        to: user.email,
        subject: "Une dernière étape pour commencer — AudiBot",
        html: emailWrapper({
          unsubscribeEmail: user.email,
          content: `
            ${h1(`${firstName ? `${firstName}, il` : "Il"} ne manque qu'une chose ☝️`)}
            ${bodyText(`Votre compte AudiBot est prêt. Mais sans l'extension Chrome, le remplissage automatique ne peut pas fonctionner.`)}
            ${bodyText(`L'installation prend <strong>2 minutes</strong>. Après ça, AudiBot remplit Almerys, Wemind, Viamedis et 10+ autres portails en un clic — à la place de vos 7 minutes de saisie manuelle.`)}
