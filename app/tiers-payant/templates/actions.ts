...
async function getSessionUser() {
  ...
  const dbUser = await prisma.user.findUnique({
    ...
    select: { id: true, role: true, plan: true, isPro: true },
  });
...
async function checkProAccess() {
  ...
  const isPro =
    ...
    user.plan === "EQUIPE" ||
    user.role === "ADMIN";
  if (!isPro) throw new Error("Disponible à partir du plan Pro");
}
...
export async function updateTemplate(
  ...
  await checkProAccess();
  return prisma.templateRelance.update({ where: { id }, data });
}
...
export async function previewTemplate(templateId: string, dossierId?: string) {
  ...
  if (dossierId) {
    ...
    if (dossier) {
      vars = {
        nom_mutuelle: String(dossier.mutuelle),
        ...
        nom_opticien: dossier.user.name || "L'opticien",
        adresse_magasin: process.env.ADRESSE_MAGASIN || "Adresse du magasin",
        date_courrier: new Date().toLocaleDateString("fr-FR"),
      };
    }
  }
