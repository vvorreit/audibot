...
import { sendRelanceEmail, buildEmailContent } from "@/lib/relance-emails";
...
async function checkAdmin() {
  ...
  if (dbUser?.role !== "ADMIN") throw new Error("Acces refuse. Reserve aux administrateurs.");
  return dbUser;
}
...
export async function executerRelance(dossierId: string, delaiJours: number, action: string) {
  ...
  /* Envoi email si action = "email" ou "both" */
  let emailSent = false;
  let emailTo: string | null = null;

  if (action === "email" || action === "both") {
    /* Determine le type d'escalade selon le nombre de relances deja envoyees */
    ...
    /* Cherche un template correspondant : d'abord par delaiJours exact, sinon par type d'escalade */
    const template = await prisma.templateRelance.findFirst({
      ...
      orderBy: [
        /* Priorite au match par delaiJours exact */
        { delaiJours: "asc" },
      ],
    });

    /* Si un template avec le bon delaiJours existe, il est prioritaire */
    let bestTemplate = template;
    if (template && template.delaiJours !== null && template.delaiJours !== delaiJours) {
      /* Le template trouve correspond au type mais pas au delai — on cherche un match exact */
...
export async function previewRelanceEmail(dossierId: string, delaiJours: number) {
  await checkAdmin();

  const dossier = await prisma.dossierTiersPayant.findUnique({
    where: { id: dossierId },
    select: { reference: true, mutuelle: true, montant: true, dateEnvoi: true, relanceCount: true },
  });
  if (!dossier) throw new Error("Dossier introuvable");

  const escalationType =
    dossier.relanceCount === 0 ? "amiable" :
    dossier.relanceCount === 1 ? "ferme" : "mise_en_demeure";

  const template = await prisma.templateRelance.findFirst({
    where: {
      actif: true,
      OR: [{ delaiJours }, { type: escalationType }],
    },
    orderBy: [{ delaiJours: "asc" }],
  });
  ...
  const content = buildEmailContent(
    { reference: dossier.reference, mutuelle: dossier.mutuelle, montant: dossier.montant, dateEnvoi: dossier.dateEnvoi },
    templateData
  );
  ...
  return {
    ...
    html: content.html,
    destinataire: `service mutuelles (${dossier.mutuelle})`,
  };
}
