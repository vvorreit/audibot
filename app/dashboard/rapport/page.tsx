"use client";
/**
 * User-facing bilan rapport dashboard: KPIs, charts, monthly trends, per-user view.
 * Exports: RapportPage (default)
 * ~520 lines
 */

import { useEffect, useState } from "react";
import { getUserBilanDashboard } from "./actions";
import {
  ...
  Target, Glasses, Heart, ChevronLeft, ChevronRight, ArrowLeft,
} from "lucide-react";
import {
...
const LABEL_MAP: Record<string, string> = {
  ...
  myopie: "Myopie", "hypermétropie": "Hypermétropie", astigmatisme: "Astigmatisme",
...
type DashboardData = Awaited<ReturnType<typeof getUserBilanDashboard>>;
...
export default function RapportPage() {
  ...
  const fetchData = async (y: number, m: number, p: Period) => {
    setLoading(true); setError(null);
    try { setData(await getUserBilanDashboard(p === "month" ? formatMonth(y, m) : undefined, p)); }
    catch (e) { setError(e instanceof Error ? e.message : "Erreur"); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchData(year, month, period); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const goMonth = (dir: -1 | 1) => {
    ...
    fetchData(ny, nm, "month");
  };
  const handlePeriod = (p: Period) => { setPeriod(p); fetchData(year, month, p); };
  ...
  if (error) return (
    <div className="text-center py-20">
      ...
      <Link href="/dashboard" className="mt-4 inline-flex items-center gap-2 text-sm text-slate-600 hover:text-slate-900">
  ...
  return (
    <DashboardShell innerClassName="space-y-0">

      {/* ── Page header ── */}
      <div className="pb-4 border-b border-slate-100 mb-6">
        <h1 className="text-3xl font-black tracking-tight text-slate-900">Rapport bilans</h1>
        <p className="text-slate-600 text-xs font-medium mt-0.5">Analytics de vos questionnaires bilan opticien</p>
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white border border-slate-100 rounded-2xl p-4">
          ...
          <div className="flex bg-slate-50 border border-slate-100 p-1 rounded-xl gap-1">
            {(["day", "week", "month"] as Period[]).map((p) => (
              <button key={p} onClick={() => handlePeriod(p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${period === p ? "bg-blue-600 text-white shadow" : "text-slate-600 hover:text-slate-700"}`}>
        ...
        {!loading && data && <span className="text-xs text-slate-400 font-semibold ml-auto">{data.totalAllTime} bilans all-time</span>}
      </div>
      ...
      {loading || !data ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <KpiCard label="Bilans créés" value={data.total} sub={`${data.delivered} complétés`}
              icon={ClipboardList} color="blue" trend={delta(data.total, data.vsLastMonth.total)} />
            <KpiCard label="Complétion" value={`${data.completionRate}%`} sub={`${data.vsLastMonth.completionRate}% mois préc.`}
              icon={CheckCircle2} color="green" />
            <KpiCard label="Complexité moy." value={data.avgComplexite} sub={`${data.vsLastMonth.avgComplexite} mois préc.`}
              icon={Activity} color="amber" />
            <KpiCard label="Opportunités / bilan" value={data.avgOpportunitesPerBilan} sub={`${data.totalOpportunites} au total`}
              icon={ShoppingCart} color="purple" />
            <KpiCard label="NPS" value={data.npsScore !== null ? data.npsScore : "—"} sub={`${data.npsResponseRate}% de réponse`}
              icon={Smile} color={data.npsScore !== null && data.npsScore >= 50 ? "green" : data.npsScore !== null && data.npsScore >= 0 ? "amber" : "red"} />
            <KpiCard label="Utilisateurs actifs" value={data.uniqueUsers} sub={`${data.topUsers.length} ce mois`}
            ...
            <div className="lg:col-span-2 bg-white p-8 rounded-card shadow-sm border border-slate-100">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart barSize={10} data={Object.entries(data.dailyVolume).map(([date, v]) => ({
                  date: date.slice(5), created: v.created, delivered: v.delivered,
                }))}>
                ...
                {[1, 2, 3, 4, 5].map((n) => {
                  ...
                  return (
                    <div key={n} title={COMPLEXITE_FULL[n]} className="text-center p-3 rounded-2xl cursor-default" style={{ backgroundColor: COMPLEXITE_BG[n] }}>
                      <p className="text-2xs font-black uppercase leading-tight" style={{ color: COMPLEXITE_COLORS[n] }}>{COMPLEXITE_LABELS[n]}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              ...
              {data.lensTopReco.length > 0 ? (
                <div className="space-y-2.5">
                  {data.lensTopReco.map((r, i) => (
                    <div key={r.label}>
                      <div className="flex justify-between text-xs font-black text-slate-600 mb-1">
                        <span className="flex items-center gap-2"><span className="text-2xs font-black text-slate-600 w-5">#{i + 1}</span>{r.label}</span>
                        <span>{r.count} <span className="font-normal text-slate-400">({r.pct}%)</span></span>
                      </div>
                      <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
              ...
              ) : <p className="text-xs text-slate-600">Aucune recommandation</p>}
            </div>
          </div>

          {/* ── NPS ── */}
          <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
            <SectionHeader icon={Smile} color={data.npsScore !== null && data.npsScore >= 50 ? "green" : "amber"}
              title="NPS client" sub={`${data.npsResponseRate}% de taux de réponse — ${data.npsCounts.promoteurs + data.npsCounts.passifs + data.npsCounts.detracteurs} réponses`} />
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                {data.npsScore !== null && (
                  <div className="p-4 rounded-2xl border border-slate-100 text-center">
                    ...
                    {data.vsLastMonth.nps !== null && <p className="text-xs text-slate-600 mt-1">{data.vsLastMonth.nps} mois précédent</p>}
                  </div>
                )}
              </div>
              <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              <SectionHeader icon={Glasses} color="blue" title="Types de correction" sub={`${data.demographics.totalWithFormData} bilans avec données`} />
              <DistribBar data={data.demographics.correctionTypes} total={data.demographics.totalWithFormData} />
              <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 gap-3">
                <div className="p-3 bg-blue-50 rounded-xl">
                  <p className="text-2xs font-black uppercase text-blue-400">Progressifs</p>
                  <p className="text-xl font-black text-blue-600">{data.demographics.progressifCount} <span className="text-xs text-slate-600">({pct(data.demographics.progressifCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
                <div className="p-3 bg-cyan-50 rounded-xl">
                  <p className="text-2xs font-black uppercase text-cyan-400">Lentilles</p>
                  <p className="text-xl font-black text-cyan-600">{data.demographics.lentillesCount} <span className="text-xs text-slate-600">({pct(data.demographics.lentillesCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
              </div>
            </div>
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              <SectionHeader icon={AlertTriangle} color="rose" title="Gênes rapportées" sub="Symptômes les plus fréquents" />
              <DistribBar data={data.demographics.genesActuelles} total={data.demographics.totalWithFormData} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              ...
              <DistribBar data={data.demographics.mainActivity} total={data.demographics.totalWithFormData} />
              <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-3">
                <div className="p-3 bg-orange-50 rounded-xl text-center">
                  <p className="text-2xs font-black uppercase text-orange-400">Écran moy.</p>
                  <p className="text-xl font-black text-orange-600">{data.demographics.avgScreenTime}h</p>
                </div>
                <div className="p-3 bg-green-50 rounded-xl text-center">
                  <p className="text-2xs font-black uppercase text-green-400">Sportifs</p>
                  <p className="text-xl font-black text-green-600">{data.demographics.sportCount} <span className="text-2xs text-slate-600">({pct(data.demographics.sportCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
                <div className="p-3 bg-slate-100 rounded-xl text-center">
                  ...
                  <p className="text-xl font-black text-slate-700">{data.demographics.conduitNuitCount} <span className="text-2xs text-slate-600">({pct(data.demographics.conduitNuitCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
              </div>
            </div>
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              <SectionHeader icon={ShoppingCart} color="green" title="Budget & projet" sub="Fourchette de budget déclarée" />
              <DistribBar data={data.demographics.budgetDistrib} total={data.demographics.totalWithFormData} />
              <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-3">
                <div className="p-3 bg-purple-50 rounded-xl text-center">
                  <p className="text-2xs font-black uppercase text-purple-400">2e paire</p>
                  <p className="text-xl font-black text-purple-600">{data.demographics.secondePaireCount} <span className="text-2xs text-slate-600">({pct(data.demographics.secondePaireCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
                <div className="p-3 bg-amber-50 rounded-xl text-center">
                  <p className="text-2xs font-black uppercase text-amber-400">Mutuelle ?</p>
                  <p className="text-xl font-black text-amber-600">{data.demographics.mutuelleInconnueCount} <span className="text-2xs text-slate-600">inconnus</span></p>
                </div>
                <div className="p-3 bg-red-50 rounded-xl text-center">
                  <p className="text-2xs font-black uppercase text-red-400">Antécédents</p>
                  <p className="text-xl font-black text-red-600">{data.demographics.antecedentsCount} <span className="text-2xs text-slate-600">({pct(data.demographics.antecedentsCount, data.demographics.totalWithFormData)}%)</span></p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              ...
              <DistribBar data={data.demographics.stylePreference} total={data.demographics.totalWithFormData} />
            </div>
            <div className="bg-white p-8 rounded-card shadow-sm border border-slate-100">
              <SectionHeader icon={Clock} color="cyan" title="Fréquence de port" />
              <DistribBar data={data.demographics.frequencePort} total={data.demographics.totalWithFormData} />
            </div>
          </div>
                  ...
                  <tbody>
                    {data.topUsers.map((u, i) => (
                      <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-3 px-4 font-bold text-slate-700">{u.storeName ?? u.name ?? "—"}</td>
